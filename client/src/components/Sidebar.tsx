...
const Sidebar = ({isOpen, setIsOpen} : {isOpen : boolean, setIsOpen: (val:boolean) => void}) => {
  ...
  const navItems = [
    ...
    return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
    ...
    <div className="p-6 pb-4">
        ...
        <nav className='flex-1 px-2 mt-2'>
            {navItems.map((item) => {
                ...
                className={`flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${isActive ? "bg-red-100 text-red-600" : "text-slate-600 hover:bg-slate-100"}`}>
                    ...
                    {item.label}
                    {isActive && <span className='ml-auto w-[5px] h-5 bg-red-500 rounded-full'></span>}
                </NavLink>
                )})
            }
        </nav>
        ...
        <div className='px-4 py-3 border-t border-slate-200'>
            <div className='flex items-center gap-3'>
                <div className='size-10 rounded-full bg-gradient-to-br from-red-500 to-pink-500 flex items-center justify-center text-white font-medium'>
            ...
            <button onClick={logout} className=" flex items-center justify-center gap-2 w-full mt-3 py-2 px-4 bg-slate-100 hover:bg-slate-200 rounded-lg transition-colors text-slate-700 font-medium text-sm">
